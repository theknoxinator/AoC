...
# Second task: Player loses 1 HP on each of their turns, everything else is the same
...
if __name__ == '__main__':
    ...
    while len(game_states) > 0:
        ...
        # Player loses 1 HP before any other effects
        ...
        if current_state.boss_hp <= 0:
            if current_state.mana_spent < smallest_mana:
                print("Newest smallest mana fight is: [{0}] with mana spent: {1!s}".format(', '.join(current_state.spell_history), current_state.mana_spent))
    ...
    # Print out answer
    print("The best winning fight is: [{0}] with mana spent: {1!s}".format(', '.join(smallest_state.spell_history), smallest_mana))
