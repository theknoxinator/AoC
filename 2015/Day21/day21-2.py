# Determine the largest amount of gold the player can spend and still lose to the boss assuming each takes
...
if __name__ == '__main__':
    ...
    # Now that we have all the possible combinations, we simulate each fight and determine which ones lose
    losing_options = []
    for option in full_options:
        ...
        while True:
            ...
            if player.is_dead():
                ...
                print("Player loses with combination: {0}, {1}, {2}, {3}"
                      .format(option[0].name, option[1].name, option[2][0].name, option[2][1].name))
                losing_options.append(option)
                break

    # Now figure out which combination costs the most
    highest_cost = 0
    highest_cost_option = None
    for option in losing_options:
        cost = option[0].cost + option[1].cost + option[2][0].cost + option[2][1].cost
        if cost > highest_cost:
            highest_cost = cost
            highest_cost_option = option

    # Print out answer
    print("The most expensive losing combination is {0}, {1}, {2}, {3} which costs: {4}"
          .format(highest_cost_option[0].name, highest_cost_option[1].name, highest_cost_option[2][0].name,
                  highest_cost_option[2][1].name, highest_cost))
